...
describe 'Wheaton/Card', ->
  ...
  it 'calls onDraw function when drawn', ->
    ...
    expect(card.instanceOptions.onDraw).toBeCalled()

  it 'cannot be played if not drawn', ->
    card.play()

    expect(card.instanceOptions.onPlay).not.toBeCalled()

  it 'calls onPlay when played after it is drawn', ->
    card.draw().play()

    expect(card.instanceOptions.onPlay).toBeCalled()

  it 'cannot be palmed if not drawn', ->
    card.palm()

    expect(card.instanceOptions.onPalm).not.toBeCalled()

  it 'calls onPalm when palmed after it is drawn', ->
    card.draw().palm()

    expect(card.instanceOptions.onPalm).toBeCalled()
  ...
  it 'calls onRevive when revived after it is drawn', ->
    ...
    expect(card.instanceOptions.onRevive).toBeCalled()
  ...
  it 'cannot be discarded if already discarded', ->
    ...
    expect(card.instanceOptions.onDiscard).not.toBeCalled()

  it 'calls onDiscard when discarded after it is drawn', ->
    card.draw().discard()

    expect(card.instanceOptions.onDiscard).toBeCalled()
  ...
  it 'can be serialized', ->
    ...
    expect(card.serialized).toEqual expectedResult
